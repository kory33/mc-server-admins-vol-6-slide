...
## 大体の流れ
 ...
 - Minecraft プロトコルの詳細
   ...
   - 例
     - ハートビート
      - クライアントは、「まだ接続してますよ」という確証をサーバーに与えるために、ハートビート (heartbeat; 心拍) と呼ばれる、定期的に送られてくるパケットに対して返事をする必要がある
   ...
   - バージョン依存性
     - TODO: 「プロトコルバージョン」があり、 Minecraft のパッチバージョンを跨ぐときにも切り替わることがあることを説明する
     - パケットIDとパケットのマッピングが一新されるので、完全に非互換になる
     - ただし、ログイン周りのパケットだけは最近のバージョンはマッピングに変化がない
